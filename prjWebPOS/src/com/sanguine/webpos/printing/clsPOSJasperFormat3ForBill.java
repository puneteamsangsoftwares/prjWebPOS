...
@Controller
public class clsPOSJasperFormat3ForBill implements clsPOSBillGenerationFormat
{
	@Autowired
	...
	public void funGenerateBill(String strBillNo, String reprint, String transactionType, String strPosCode, String strBillDate, String strClientCode, String strServerBillPrinterName, boolean isOriginal)
	{
		...
		try
		{
			...
			/*
			 * sql.append(
			 * "select a.strMultipleBillPrinting, a.strBillFormatType, a.strScanQRYN, a.blobReportImage, a.strReprintOnSettleBill,"
			 * + "FROM tblsetup a" + "WHERE a.strPOSCode='P01'"); list =
			 * objBaseService.funGetList(sql, "sql"); if(list!=null) { for(int
			 * i=0;i<list.size();i++) { Object[] obj=(Object[])list.get(i);
			 * multiBillPrint = (boolean) obj[0]; billFormatType =
			 * obj[1].toString(); reprintOnSettleBill = (boolean) obj[4]; }
			 * 
			 * }
			 */
			...
			String sql_BillDtl = "select sum(a.dblQuantity),a.strItemName as ItemLine1" + " ,a.strItemName as ItemLine2,sum(a.dblAmount),a.strItemCode,a.strKOTNo ,b.strItemType" + " from " + billdtl + " a,tblitemmaster b  " + " where a.strBillNo='" + strBillNo + "'" + " and a.strItemCode=b.strItemCode " + " and a.tdhYN='N' and date(a.dteBillDate)='" + strBillDate + "'";
			...
			if (list.size() > 0)
			{
				...
				for (int i = 0; i < list.size(); i++)
				{
					...
					//String itemType=(String) list.get(6);
					String itemType=objBillDetail[6].toString();
				...
				String sqlPromoBills = "select dblQuantity from " + billPromoDtl + " " + " where strBillNo='" + strBillNo + "' and strItemCode='" + objBillDetail[4].toString() + "' " + " and date(dteBillDate)='" + strBillDate + "' ";
				List listPromoItems = objBaseService.funGetList(new StringBuilder(sqlPromoBills), "sql");
				if (listPromoItems.size() > 0)
				{
					for (int j = 0; j < listPromoItems.size(); i++)
					{
				...
				if (saleQty > 0)
				{
					...
					listGeneralBillDtl.add(objPOSBillDtl);
					...
					if (cntRecord > 0)
					{
						...
						while (listModifier.size() > 0)
						{
							...
							{
								for (int k = 0; k < listModifier.size(); i++)
								{
			...
			JasperDesign jd = JRXmlLoader.load(servletContext.getResourceAsStream(("/WEB-INF/reports/billFormat/rptBillFormat3ForHDJasperBillPrint.jrxml")));
