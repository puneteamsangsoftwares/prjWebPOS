...
@Controller
public class clsPOSJasperFormat8ForBill implements clsPOSBillGenerationFormat
{
	@Autowired
	...
	public void funGenerateBill(String strBillNo,String reprint, String transactionType,String strPosCode, String strBillDate,String strClientCode,String strServerBillPrinterName, boolean isOriginal)
    {
		...
		try
		{
			...
			boolean multiBillPrint = false;
			boolean reprintOnSettleBill = false;

			boolean flgComplimentaryBill = false;
			String billNo="",tableName="",waiterNo="",billDate="",operationType="",strUserName="",strCustomerCode="",strBillType="",billFormatType="",strPrintOrderNoOnBillYN="";
			...
			/*sql.append("select a.strMultipleBillPrinting, a.strBillFormatType, a.strScanQRYN, a.blobReportImage, a.strReprintOnSettleBill,"
			+ "FROM tblsetup a"
			+ "WHERE a.strPOSCode='P01'");	
		    ...
		    if(list!=null)
		    {
		    	for(int i=0;i<list.size();i++)
		    	{
		    		...
		    		multiBillPrint = (boolean) obj[0];
		    		billFormatType = obj[1].toString();
		    		reprintOnSettleBill = (boolean) obj[4];
		    	}
			...
			sql.append(" select a.strBillNo,ifnull(b.strTableName,''),ifnull(c.strWShortName,''),a.dblGrandTotal,a.dblSubTotal,"
					+ " a.dblDiscountAmt,a.dteBillDate,a.intPaxNo,a.strOperationType,a.strCustomerCode,a.strUserCreated "
					+ " from tblbillhd a left outer join tbltablemaster b on a.strTableNo=b.strTableNo left outer join tblwaitermaster c "
					+ "on a.strWaiterNo=c.strWaiterNo " + " where a.strBillNo='" + strBillNo + "' " + " and a.strPosCode='" + strPosCode + "' ");
		    list = objBaseService.funGetList(sql, "sql");
		    if(list!=null)
		    {
		    	for(int i=0;i<list.size();i++)
            ...
            String sql_HomeDelivery = "select strBillNo,strCustomerCode,strDPCode,tmeTime,strCustAddressLine1 "
                    + "from tblhomedelivery where strBillNo='"+strBillNo+"' ;";
            ...
            if (listHomeDel!=null)
            {
                ...
                if (operationType.equalsIgnoreCase("TakeAway")){
                    ...
                    String sqlTakeAway = "select a.strBillNo,a.dteBillDate,a.strCustomerCode,b.strCustomerName,b.longMobileNo "
			...
			String sqlSettle = "select b.strSettelmentType from " + billSettlementdtl + " a,tblsettelmenthd b "
                    + " where a.strSettlementCode=b.strSettelmentCode and a.strBillNo='" + strBillNo + "' and b.strSettelmentType='Complementary' "
                    + " and date(a.dteBillDate)='" + strBillDate + "'";
			...
			if (funIsDirectBillerBill(strBillNo, billhd))
            {
                ...
                sql_BillHD = "select a.dteBillDate,time(a.dteBillDate),a.dblDiscountAmt,a.dblSubTotal,"
                        ...
                        + "where a.strBillNo='"+strBillNo+"'  and date(a.dteBillDate)='"+strBillDate+"' ";
			...
			else
			{
				sql_BillHD = "select a.strTableNo,a.strWaiterNo,a.dteBillDate,time(a.dteBillDate),a.dblDiscountAmt,a.dblSubTotal,"
                        ...
                        + "where a.strBillNo='"+strBillNo+"' and b.strOperational='Y' and date(a.dteBillDate)='"+strBillDate+"' ";
				list = objBaseService.funGetList(new StringBuilder(sql_BillHD), "sql");
				if(list.size()>0)
				{
					for(int i=0;i<list.size();i++)
					...
					else
                    {
                        ...
                        query = "SELECT strWShortName FROM tblwaitermaster WHERE strWaiterNo='"+waiterNo+"' "
                        		+ "AND (strPOSCode='"+strPosCode+"' or strPOSCode='All'); ";
            ...
            String sql_BillDtl = "select sum(a.dblQuantity),a.strItemName as ItemLine1"
                    + " ,a.strItemName as ItemLine2,sum(a.dblAmount),a.strItemCode,a.strKOTNo "
                    + " from " + billdtl + " a "
                    + " where a.strBillNo='"+strBillNo+"' and a.tdhYN='N' and date(a.dteBillDate)='"+strBillDate+"'";
            ...
            if(list.size()>0)
            {
                ...
                String sqlPromoBills = "select dblQuantity from " + billPromoDtl + " "
                        + " where strBillNo='" + strBillNo + "' and strItemCode='" + objBillDetail[4].toString() + "' "
                ...
                if (listPromoItems.size()>0)
                {
                	...
                	{
                		objBillDetail=(Object[])listPromoItems.get(i);
                	}
                ...
                if (saleQty > 0)
                {
                    ...
                    String sqlModifier = "select count(*) "
                            + "from " + billModifierdtl + " where strBillNo='"+strBillNo+"' and left(strItemCode,7)='"+objBillDetail[4].toString()+"' "
                    ...
                    if (cntRecord > 0)
                    {
                        sqlModifier = "select strModifierName,dblQuantity,dblAmount "
                                ...
                                + " where strBillNo='"+strBillNo+"' and left(strItemCode,7)='"+objBillDetail[4].toString()+"' "
                                + " and date(dteBillDate)='" + strBillDate + "'";
                        if (!strPrintZeroAmtModifierOnBill.equals("N"))
                        {
                            sqlModifier += " and  dblAmount !=0.00 ";
                        ...
                        while (listModifier.size()>0)
                        {
                            ...
                            if (flgComplimentaryBill)
                            {
                            	...
                            	{
                            		objMod = (Object[])listModifier.get(i);
                            	}
            ...
            String sqlDisc = "select a.dblDiscPer,a.dblDiscAmt,a.strDiscOnType,a.strDiscOnValue,b.strReasonName,a.strDiscRemarks "
                    + "from " + billDiscount + " a ,tblreasonmaster b "
            ...
            String sql_Tax = "select b.strTaxDesc,sum(a.dblTaxAmount),b.strBillNote "
                    ...
                    + " where a.strBillNo='" + strBillNo + "' and a.strTaxCode=b.strTaxCode "
            ...
            String sqlSettlementBreakup = "select a.dblSettlementAmt, b.strSettelmentDesc, b.strSettelmentType "
                    ...
                    + "where a.strBillNo='"+strBillNo+"' and a.strSettlementCode=b.strSettelmentCode"
                    + " and date(a.dteBillDate)='" + strBillDate + "'";
