...
@Controller
public class clsPOSHourlyItemWiseSalesReport
{
	@Autowired
	...
	@RequestMapping(value = "/frmHourlyItemWiseReport", method = RequestMethod.GET)
	public ModelAndView funOpenForm(Map<String, Object> model,HttpServletRequest request)throws Exception
	{
		...
		if("2".equalsIgnoreCase(urlHits)){
			return new ModelAndView("frmHourlyItemWiseReport_1","command", new clsPOSReportBean());
		}else if("1".equalsIgnoreCase(urlHits)){
			return new ModelAndView("frmHourlyItemWiseReport","command", new clsPOSReportBean());
		}else {
			return null;
		}
	...
	@RequestMapping(value = "/rptHourlyItemWiseSalesReport", method = RequestMethod.POST)	
	public void funReport(@ModelAttribute("command") clsPOSReportBean objBean, HttpServletResponse resp,HttpServletRequest req,String source)
	{
		try
		{
			String reportName = servletContext.getRealPath("/WEB-INF/reports/webpos/rptHourlyItemWiseSalesReport.jrxml");
			...
			List listHourlyItem= new ArrayList();
			String fromDate = hm.get("fromDate").toString();
			String toDate = hm.get("toDate").toString();
			String strUserCode = hm.get("userName").toString();
			String strPOSCode = posCode;
			String strShiftNo = "1";
			listHourlyItem = objReportService.funProcessHourlyItemWiseReport(posCode,fromDate,toDate,strShiftNo);

            JasperDesign jd = JRXmlLoader.load(reportName);
			JasperReport jr = JasperCompileManager.compileReport(jd);
			List<JasperPrint> jprintlist = new ArrayList<JasperPrint>();
			JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(listHourlyItem);
