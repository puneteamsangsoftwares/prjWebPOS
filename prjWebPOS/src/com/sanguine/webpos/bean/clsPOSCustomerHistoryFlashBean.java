package com.sanguine.webpos.bean;

public class clsPOSCustomerHistoryFlashBean 
{
	
	private String strPOSName;
	private String dteFromDate;
	private String dteToDate;
	private String strAmount,strAmt;
	private String strCustCode;
	private String strCustName;
	private String strTabVal;
	private String strReportType;
	
	public String getStrPOSName() {
		return strPOSName;
	}
	public void setStrPOSName(String strPOSName) {
		this.strPOSName = strPOSName;
	}
	public String getDteFromDate() {
		return dteFromDate;
	}
	public void setDteFromDate(String dteFromDate) {
		this.dteFromDate = dteFromDate;
	}
	public String getDteToDate() {
		return dteToDate;
	}
	public void setDteToDate(String dteToDate) {
		this.dteToDate = dteToDate;
	}
	public String getStrAmount() {
		return strAmount;
	}
	public void setStrAmount(String strAmount) {
		this.strAmount = strAmount;
	}
	public String getStrAmt() {
		return strAmt;
	}
	public void setStrAmt(String strAmt) {
		this.strAmt = strAmt;
	}
	public String getStrCustCode() {
		return strCustCode;
	}
	public void setStrCustCode(String strCustCode) {
		this.strCustCode = strCustCode;
	}
	public String getStrCustName() {
		return strCustName;
	}
	public void setStrCustName(String strCustName) {
		this.strCustName = strCustName;
	}
	public String getStrTabVal() {
		return strTabVal;
	}
	public void setStrTabVal(String strTabVal) {
		this.strTabVal = strTabVal;
	}
	public String getStrReportType() {
		return strReportType;
	}
	public void setStrReportType(String strReportType) {
		this.strReportType = strReportType;
	}
	
	
	
}
