...
<%}
	...
	function funGetSelectedRowIndex(obj, tableId) {
		...
		{
			var billNo = code;
			funFillBillAmountAndLooseCash(billNo);
...
// 				document.getElementById(code).style.backgroundColor = 'gray';
	...
	function funFillBillAmountAndLooseCash(billNo)
	{
		var searchurl = getContextPath() + "/funSetBillAmountAndLooseCash.html?billNo="+billNo;
		$.ajax({
			type : "GET",
			...
			async : false,
			success : function(response) {
				$("#billAmount").text(response.dblGrandTotal);
...
<body>
	<div id="formHeading">
		<label>Assign Home Deliveryr</label>
	</div>
	<s:form name="AssignHomeDeliveryForm" method="POST"
		...
		<div style="width: 100%; margin-left: 95px">
			<table>
				...
				<tr>		
					<td>Bill Amount :</td>
					<td width="30%"><label for="BillAmount" id="billAmount"></label></td> 
					
					<td>Loose Cash :</td>
					<td><s:input type="text" id="txtLooseCash" 
						name="txtLooseCash" path="dblLooseCash" value="0.00"
